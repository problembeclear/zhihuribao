...
@implementation CollectScrollView
//主布局
- (void) LayoutSelf {
    ...
    if ([self.collectionDatabase open]) {
        ...
        while([resultSet next]) {
            ...
            NSString* title = [resultSet stringForColumn:@"labelStr"];
            NSString* image = [resultSet stringForColumn:@"imageURL"];
            if (URL != nil) {
    ...
    self.idStr = self.arrayForButtonState[self.page - 1];
...
- (void) LayoutScrollView {
    ...
    self.scrollView.contentSize = CGSizeMake(Width * self.arrayForButtonState.count, 0);
    ...
    for (int i = 0; i < self.arrayForButtonState.count; i++) {
        ...
        //
        NSString* URLString = [NSString stringWithString:self.arrayForButtonState[i]];
...
- (void) scrollViewDidEndDecelerating:(UIScrollView *)scrollView {
    ...
    NSLog(@"%@", self.arrayForButtonState);
...
- (void) returnToMainView {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"returnToCollectionView" object:nil];
}
//进入评论页面
- (void) pushToComment {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"pushToScrollComment" object:nil userInfo:@{@"url":self.arrayForButtonState[page] }];
}
...
- (void) collect {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:@"collectByScrollView" object:nil userInfo:@{@"url":self.idStr,@"title":self.titleArray[self.page - 1],@"imageURL":self.imageArray[self.page - 1],@"selected":stringSelected, @"index":indexString }];
